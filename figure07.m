...
ens = 'passive';
file = [DIR, '\Processed_Data\', ens, '-SP-Final-JUNE-', ...
...
for ii = 1:length(months)
    ...
    eval(['Qu = squeeze(nanmean(Quz100_', months{ii}, '(:,:,1:6),3));'])
    eval(['Qv = squeeze(nanmean(Qvz100_', months{ii}, '(:,:,1:6),3));'])
...
print('-djpeg', '-r300', ...
    ['./Figure_05_4812_transport_', ...
    num2str(target_year), '_', ens, '_SP_100m'])   